...
let
  ...
  rj-notifier = pkgs.writeShellApplication {
    name = "rj-notifier";
    ...
    text = ''
      ...
      # for the given day                          #
      ##############################################
      departure_time="2025-08-21T19:00:00.000+02:00"
      ##############################################
      ...
      while true; do
        ...
        n_seats_available="$(curl \
          ...
          -H 'Pragma: no-cache' \
          -H 'TE: trailers' \
          | jq ".routes | .[] | {seats: .freeSeatsCount, time: .departureTime} | select(.time == \"$departure_time\") | .seats"
        )";
